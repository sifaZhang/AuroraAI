(() => {
  ...
  const renderRows = items => {
    $('head').innerHTML = '<tr>' + ['\u80a1\u7968', '\u884c\u4e1a', '\u7c7b\u578b', ...years, '\u4e09\u5e74\u5e73\u5747 DPS', '\u52a0\u5165\u6765\u6e90', '\u72b6\u6001', '\u64cd\u4f5c'].map(item => `<th>${item}</th>`).join('') + '</tr>';
    $('rows').innerHTML = items.map(item => `<tr><td class="stock-cell"><strong>${item.company_name}</strong><small>${item.symbol}</small></td><td>${cell(item.industry_level_1)}<small>${cell(item.industry_level_2)}</small></td><td><span class="dividend-tag ${item.stability_subtype === 'resource_monopoly_cyclical' ? 'cyclical' : ''}" title="${item.risk_note || ''}">${subtype[item.stability_subtype] || item.stability_subtype}</span></td>${years.map(year => `<td class="dps">${dps(item.annual_dps[year])}</td>`).join('')}<td class="dps">${dps(item.three_year_average_dps)}</td><td><span class="source-tag">${source[item.inclusion_source] || item.inclusion_source}</span></td><td><span class="status-chip ${item.is_enabled ? 'eligible' : 'neutral'}">${item.is_enabled ? text.enabled : text.disabled}</span></td><td><button class="detail-button" data-symbol="${item.symbol}" data-enabled="${item.is_enabled}">${item.is_enabled ? '\u505c\u7528' : '\u91cd\u65b0\u542f\u7528'}</button></td></tr>`).join('');
  ...
  async function load() {
    ...
    $('rows').innerHTML = '<tr><td colspan="10" class="empty-state">' + text.loading + '</td></tr>';
    ...
    try {
      ...
      renderRows(data.items); $('message').textContent = `\u5171 ${data.total} \u53ea\uff0c\u542f\u7528 ${data.enabled_count} \u53ea\u3002`;
