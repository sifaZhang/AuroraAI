  ...
  function createPage(options) {
    ...
    function renderCandidateDetail(detail) {
      ...
      [
        ...
        ["二级/三级行业", [candidate.industry_level2, candidate.industry_level3].filter(Boolean).join(" / ") || null],
        ["有效行业", candidate.effective_industry_code],
        ["尾盘行业分/排名", candidate.intraday_industry_score == null ? null : `${candidate.intraday_industry_score} / ${candidate.intraday_industry_rank ?? "—"}`],
        ["正式行业分/排名", candidate.official_industry_score == null ? null : `${candidate.official_industry_score} / ${candidate.official_industry_rank ?? "—"}`],
        ["资金/龙头", candidate.capital_activity_score == null ? null : `${candidate.capital_activity_score} / ${candidate.leader_score ?? "—"}`],
        ["尾盘总分/等级", candidate.intraday_total_score == null ? null : `${candidate.intraday_total_score} / ${candidate.intraday_candidate_grade ?? "—"}`],
        ["最终总分/等级", candidate.final_total_score == null ? null : `${candidate.final_total_score} / ${candidate.final_candidate_grade ?? "—"}`],
        ["最终建议", candidate.final_buy_recommendation],
        ["确认状态/变化", [candidate.confirmation_status, candidate.confirmation_change_type].filter(Boolean).join(" / ") || null],
