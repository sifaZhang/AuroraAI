...
def plan(connection, calculation_date: date):
    ...
    totals, counts = aggregate_events(provider.fetch_events(row["symbol"] for row in securities), years)
    ...
    return years, rows, totals, counts, missing


def apply(connection, calculation_date: date, years, rows, totals, counts) -> None:
    ...
    with connection:
        for row in rows:
            for year in years:
                ...
                connection.execute("""INSERT INTO annual_cash_dividend_summaries(market,symbol,calendar_year,cash_dividend_per_share,dividend_event_count,calculation_method,source,data_quality_status,calculated_at,updated_at) VALUES('CN',?,?,?,?,?,?,?,?,?) ON CONFLICT(market,symbol,calendar_year) DO UPDATE SET cash_dividend_per_share=excluded.cash_dividend_per_share,dividend_event_count=excluded.dividend_event_count,calculation_method=excluded.calculation_method,source=excluded.source,data_quality_status=excluded.data_quality_status,calculated_at=excluded.calculated_at,updated_at=excluded.updated_at""", (row["symbol"], year, value, count, METHOD, "tushare", "complete", now, now))
...
def main() -> int:
    ...
    try:
        years, rows, totals, counts, missing = plan(connection, args.calculation_date)
        ...
        if args.apply: apply(connection, args.calculation_date, years, rows, totals, counts)
