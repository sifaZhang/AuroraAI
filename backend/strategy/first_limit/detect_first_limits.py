...
def detect_first_limits(con, *, start, end, codes, detection_version='first_limit_v1',
 ...
 def decide(symbol,day):
  ...
  return (symbol,day,detection_version,decision,target['open'],target['high'],target['low'],target['close'],fetch(day)['pre_close'] if fetch(day) else None,str(decision.upper_limit) if decision.upper_limit else None,None)
