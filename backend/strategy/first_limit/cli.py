...
def main(argv=None):
    ...
    quality=connection.execute("""SELECT * FROM first_limit_quality_scores WHERE event_id=?
        AND scoring_version='first_limit_quality_v1' ORDER BY id DESC LIMIT 1""",(event["id"],)).fetchone()
    pullback=connection.execute("""SELECT * FROM first_limit_pullback_observations WHERE event_id=?
        AND observation_date<=? ORDER BY observation_date DESC,id DESC LIMIT 1""",(event["id"],args.trade_date)).fetchone()
    ...
    result=FirstLimitCandidateScoringService(connection).score(
        event,dict(quality) if quality else None,dict(pullback) if pullback else None,industry,
        f"{args.trade_date}T{args.as_of_time}:00+08:00",[event["symbol"]])
