...
const ids={{}}; for (const id of ['sector-rows','radar-error','industry-scroll-top','industry-table-wrap','industry-refresh-status','industry-refresh-button','industry-search','industry-sort','industry-order','detail-close','industry-detail','detail-content','industry-trade-date','constituents-content']) ids[id]=new El();
...
radar.state.level=3;radar.state.items=[{{industry_code:'850531',industry_name:'稀土',parent_code:'801050',total_score:95.67,rank_in_level:3}}];radar.render();
...
radar.state.level=1;radar.render();ids['sector-rows'].children[0].onclick();await new Promise(resolve=>setImmediate(resolve));assert.strictEqual(ids['industry-detail'].shown,2);
