...
def test_market_pulse_page_contract_and_navigation():
    ...
    for element_id in ("refresh-sectors", "sector-rows", "source-statuses", "refresh-job-card", "job-progress", "sector-search", "level-filter", "source-filter", "reset-sector-filters", "breadth-available", "approximate-count"):
        assert f'id="{element_id}"' in page
    assert page.count('class="sort-button') == 10
...
def test_market_pulse_frontend_scope_and_safety():
    ...
    assert "breadth_score" in script and "total_score" in script
    assert "breadth_metrics" in script and "lookahead_warning" in script
    assert "BREADTH_STATUS_NAMES" in script and "METRIC_NAMES" in script
    assert "70分趋势与30分市场广度" in page
