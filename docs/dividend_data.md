...
```powershell
python -m backend.collector.collect_dividends --limit 200 --output data\dividend_top20.csv
```
...
date has not passed, and then calculates Dividend Top20. It does not scan every
A-share one by one.
