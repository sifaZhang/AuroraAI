...
## 2026-08-14: 单股预期差刷新日期（已实现，待提交）

- 预期差表“数据日期”现在包含最新股价时间；A 股单股价格刷新与港股单股价格/评级刷新完成后都会同步更新该列。

## 2026-08-14: 单股港股评级刷新（已实现，待提交）

- 预期差表中每只港股可单独强制刷新晨星和分析师评级，不再需要等待全市场评级任务。
- 刷新任务表迁移允许保存 `HK.xxxxx` 单股来源；旧任务来源会被保留。
...
- 同一按钮现在也批量刷新已保存高股息候选池的价格和当前收益率；历史 DPS、历史收益率与候选资格保持不变，完整重筛仍由“重新筛选候选池”单独执行。
- 修复候选池刷新后的前端展示：价格刷新完成后页面会同时重新读取正式分红池与候选池，避免候选池文件已更新但页面仍显示旧结果。
- 候选池标题保留完整扫描时间，同时单独显示最近价格刷新时间和实际价格日期，避免把不变的历史扫描时间误认为价格没有更新。
